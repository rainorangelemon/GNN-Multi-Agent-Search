...
from expert.ECBS import ECBSPlanner, pad_paths
from torch_geometric.data import Batch


class EvaluatorECBS(BaseAgent):
    ...
    def test(self, mode):
        ...
        (data_path / 'result_ECBS' / '').mkdir(parents=True, exist_ok=True)
        ...
        for data in dataloader:
            ...
            try:
                ...
                planner = ECBSPlanner()
                optimal_node = planner.plan(self.Env_Simulator, starts, goals,
                                                time_budget=self.config.timeout,
                                                w=self.config.infer_w)
            ...
            if optimal_node is not None:
                ...
                if self.config.infer_w!=float('inf'):
                    suffix = f'_w_{self.config.infer_w}'
                else:
                    suffix = ''
                
                with open(data_path/ 'result_ECBS' /f'{name_prefix}{suffix}.ecbs', 'wb') as file:
                    ecbs_result = {
                        "ID_map": ID_map,
                        "ID_graph": ID_graph,
                        'ID_case': ID_case,
                    ...
                    pickle.dump(ecbs_result, file, protocol=pickle.HIGHEST_PROTOCOL)
